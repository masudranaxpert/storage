...
func TestExtractFilenameFromHeaderAndURL(t *testing.T) {
	...
	name1 := ExtractFilenameFromHeaderAndURL(h1, "http://example.com/download?id=123")
	if name1 != "avengers_endgame.mp4" {
		t.Errorf("expected 'avengers_endgame.mp4', got '%s'", name1)
	}

	// Case 2: URL fallback with query params
	name2 := ExtractFilenameFromHeaderAndURL(nil, "https://cdn.example.com/videos/stream_4k_test.mkv?token=abc123xyz")
...
func TestMIMEDetectionRejectsNonVideo(t *testing.T) {
	...
	err = ValidateVideoFile(fakePath)
	if err == nil {
